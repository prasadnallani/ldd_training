...
static ssize_t char_dev_read(struct file *file, char *buf, size_t lbuf,
			     loff_t * ppos)
{
...
static struct file_operations char_dev_fops = {.owner =
	    THIS_MODULE,.read = char_dev_read,.write =
	    char_dev_write,.open = char_dev_open,.release =
	    char_dev_release,.llseek = char_dev_lseek
};
